// 11/10/2021 17:44 - CHEGADA NA CIDADE OU FILIAL DE DESTINO - ("CONFIRMA FACIL V2")
...
const sqlFileName    =  path.join(__dirname, '../../sql/CF2/consultas/chegada_na_cidade_ou_filial_destino.SQL')
    ...
    for await (cli of listCli ) {                                 // Loop Clientes CF
        ...
        if(!ret.success) {
          if(ret.rowsAffected == -1 ) {
              console.log(moment().format(),'V2 - FALHA (chegadaNF.js) RET:',ret)    
          }   
        }    
    }
    return retorno
